...
int main(void)
{
	...
	Font unifont = LoadFont("resources/unifont.otf");
	...
	Vector2 documentationPos = {50, 50};
	char *documentation = LoadFileText("resources/documentation.txt");
	...
	while(!WindowShouldClose())
	{
		...
		{			
			...
			DrawRectangle(50, 320, 400, 20, GRAY);
			Vector2 errorPos = {52, 322};
			DrawTextEx(unifont, errorMsg, errorPos, 16, 1, WHITE);
			...
			Vector2 msPos = GetTouchPosition(0);
			...
			Rectangle upArrow = {455, 50, 20, 20};
			DrawRectangleRec(upArrow, ORANGE);
			
			Rectangle downArrow = {455, 75, 20, 20};
			DrawRectangleRec(downArrow, ORANGE);
			
			if(list->next != NULL)
			{
				if(IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(msPos, upArrow) && part->next->next != NULL)
				{
					y -= 20;
				...
				if(IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(msPos, downArrow) && list != part && list->next != NULL)
				{
					y += 20;
					list = setOffset(list, x, y);
			...
			Rectangle codeBox = {50, 50, 400, 260};
			Vector2 startBox = {52, 52};
			if(list->next == NULL) DrawTextEx(unifont, "Drag code here!", startBox, 16, 1, WHITE);
			if(btnEnable == 1 && IsMouseButtonReleased(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(msPos, codeBox) && list->next == NULL)
			{
				list = insertNext(list, stmt, unifont, NULL, NULL, NULL, NULL);
				list = setOffset(list, x, y);
			}
			...
			**********************************/
			...
			if(IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(msPos, startBtnRec) && ColorIsEqual(startBtnClr, GREEN))
			{
				...
				int fail = parse(&code, &tk, &st, &saveTb, &saveTableSize, &instList, &instListSize, &label, &halt, &errorMsg);
		...
		} else if(state == 1)
		{
			...
			Rectangle upArrow = {725, 50, 20, 20};
			DrawRectangleRec(upArrow, ORANGE);
			
			Rectangle downArrow = {725, 75, 20, 20};
			DrawRectangleRec(downArrow, ORANGE);
			
			DrawTextEx(unifont, documentation, documentationPos, 16, 1, BLACK);
			
			if(IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(msPos, upArrow))
			{
				documentationPos.y -= 20;
			}   
			
			if(IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(msPos, downArrow))
			{
				documentationPos.y += 20;
			}
			...
			DrawRectangle(0, 0, 750, codeHeight+10, RAYWHITE);
